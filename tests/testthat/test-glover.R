...
aquifer_drawdown_ratio <- get_aquifer_drawdown_ratio(distance = distance, K = K, D = D, V = V, t = t)
# # For pasting results into expect_equal()
...
aquifer_drawdown_ratio_df <- get_aquifer_drawdown_ratio(tibble(distance = distance, K = K, D = D, V = V, t = t))
...
pumping_depletion <- get_depletion_from_pumping(x1 = x1, x2 = x2, y_diff = y_diff, K = K, D = D, V = V, t = t) %>%
...
pumping_depletion_df <- get_depletion_from_pumping(tibble(x1 = x1, x2 = x2, y_diff = y_diff, K = K, D = D, V = V, t = t)) %>%
  dplyr::mutate(dplyr::across(dplyr::everything(), function(x) round(as.numeric(x), 4)))
...
aquifer_drawdown_ratio <- get_aquifer_drawdown_ratio(distance = distance, K = K, D = D, V = V, t = t, well_diam = well_d)
...
aquifer_drawdown_ratio <- get_aquifer_drawdown_ratio(distance = distance, K = K, D = D, V = V, t = t, well_diam = rep(well_d, length(distance)))
