...
- `along_stream_distance`: Component of the distance between the two wells that
...
obs_wells <- tibble(along_stream_distance = c(0, 1.5), x2 = c(0.5),
                    `Obs. well` = c("a", "b"))

locations <- crossing(pump_wells, obs_wells)%>%
  mutate(across(all_of(c("x1", "x2", "along_stream_distance")),
                function(x) set_units(x, "mi")), id = row_number())
```
...
ggplot() + 
  ...
  geom_point(data = obs_wells, aes(x2, y = along_stream_distance,
                                   shape = `Obs. well`), size = 4) +
  scale_shape_manual(values = c(0, 5)) +
  xlab("Distance from river (x)") + ylab("Along-stream distance") +
...
df_prep <- tibble(t = set_units(elapsed_time_years, "year"),
             ...
             along_stream_distance = set_units(0.01, "mi"),
