...
- `y_diff`: Component of the distance between the two wells that
...
obs_wells <- tibble(y_diff = c(0, 1.5), x2 = c(0.5),
                    `Obs. well` = c("a", "b"))

locations <- crossing(pump_wells, obs_wells)%>%
  mutate(across(all_of(c("x1", "x2", "y_diff")),
...
ggplot() + 
  ...
  geom_point(data = obs_wells, aes(x2, y = y_diff,
