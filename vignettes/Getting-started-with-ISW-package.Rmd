...
The `isw` package has four analytical functions to understand interconnected surface waters:
...
* `get_straight_stream_drawdown_ratio`: Estimate aquifer drawdown using a straight-stream image well
...
* `y_diff`: distance between observation and pumping wells parallel to the
  stream.
  *parallel* to the river.

For calculations without a stream, only the direct well-to-observation
`distance` is needed.
...
at an observation well (*without a stream*). The `distance` parameter is the
direct distance between the pumping and observation wells.

```{r}
distance <- set_units(1, "mi")
aquifer_drawdown_ratio <- get_aquifer_drawdown_ratio(
  distance = distance,
  K = K, D = D, V = V, t = t
)
...
Note that `get_aquifer_drawdown_ratio` has an optional parameter, `well_diam` that defaults to a value of 0. When specified, drawdown does not increase inside the well. In other words, when `distance < well_diam/2`, drawdown is calculated as if `distance = well_diam/2`.
...
the pumping and observation wells, `y_diff` is the component of
this distance that is parallel to the stream. In other words,
$r^2 = (x_2-x_1)^2 + y_{diff}^2$. Keeping the remaining
...
```{r}
distance <- set_units(seq(1/5280, 10, length.out = 200), "mi")
t <- set_units(2, 'year')
...
df <- tibble(K = K, D = D, V = V, t = t) %>%
  crossing(distance = set_units(seq(1/5280, 10, length.out = 200), "mi"))
...
ggplot(drawdown_df, aes(distance, drawdown)) +
  geom_line()
```
...
```{r message = FALSE}
d_distance <- distance[2] - distance[1]
drawdown_df <- drawdown_df %>% 
  mutate(d_area = d_distance * 2 * pi * distance,
...
```{r}
drawdown_df <- drawdown_df %>% 
  mutate(flow_towards_well = 2 * pi * distance * D * K *
           (drawdown - lag(drawdown)) / d_distance,
         flow_towards_well = set_units(flow_towards_well, "ft^3/s"))

ggplot(drawdown_df, aes(distance, flow_towards_well)) + geom_line() +
  geom_abline(slope = 0, intercept = 1, color = "red", linetype = "dashed")
```

Note that closer to the pumping well this calculation suffers from coarse discretization of the domain causing the flowrate to diverge from 1 ft^3^/s. You can verify this yourself by, for instance, changing the radius to `distance <- set_units(seq(1, 5, length.out = 200), "ft")` and re-running the calculations.
