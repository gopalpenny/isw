...
* `along_stream_distance`: distance between observation and pumping wells
...
the pumping and observation wells, `along_stream_distance` is the component of
this distance that is parallel to the stream. In other words,
$r^2 = (x_2-x_1)^2 + \mathrm{along\_stream\_distance}^2$. Keeping the remaining
...
```{r}
along_stream_distance <- set_units(seq(1/5280, 10, length.out = 200), "mi")
t <- set_units(2, 'year')
...
df <- tibble(K = K, D = D, V = V, t = t, x1 = Inf, x2 = Inf) %>%
  crossing(along_stream_distance = set_units(seq(1/5280, 10, length.out = 200), "mi"))
...
ggplot(drawdown_df, aes(along_stream_distance, drawdown)) +
  geom_line()
```
...
```{r message = FALSE}
d_along_stream_distance <- along_stream_distance[2] - along_stream_distance[1]
drawdown_df <- drawdown_df %>% 
  mutate(d_area = d_along_stream_distance * 2 * pi * along_stream_distance,
...
```{r}
drawdown_df <- drawdown_df %>% 
  mutate(flow_towards_well = 2 * pi * along_stream_distance * D * K *
           (drawdown - lag(drawdown)) / d_along_stream_distance,
         flow_towards_well = set_units(flow_towards_well, "ft^3/s"))

ggplot(drawdown_df, aes(along_stream_distance, flow_towards_well)) + geom_line() +
  geom_abline(slope = 0, intercept = 1, color = "red", linetype = "dashed")
```

Note that closer to the pumping well this calculation suffers from coarse discretization of the domain causing the flowrate to diverge from 1 ft^3^/s. You can verify this yourself by, for instance, changing the radius to `along_stream_distance <- set_units(seq(1, 5, length.out = 200), "ft")` and re-running the calculations.
