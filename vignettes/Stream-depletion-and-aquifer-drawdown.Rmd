...
df <- crossing(x1 = set_units(seq(0, 4, by = 0.02) * 5280,'ft'),
               along_stream_distance = set_units(seq(0, 3, by = 0.02) * 5280,'ft'),
...
```{r}
# df2 %>% dplyr::select(x1, along_stream_distance, t, s_ratio) %>%
#   pivot_wider(names_from = "t", values_from = "s_ratio")
...
p_drawdown <- ggplot(df2 %>% dplyr::filter(aquifer_drawdown_ratio <= set_units(-1/100/12,'s/ft^2'))) +
  geom_raster(aes(as.numeric(x1/5280), as.numeric(along_stream_distance/5280), fill = -log(-as.numeric(aquifer_drawdown_ratio)))) +
  # geom_point(data = df2 %>% dplyr::filter(s_ratio <= -1/100/12, t == t_sec*10),
  #            aes(x1/5280, along_stream_distance/5280, color = -log(-s_ratio))) +
...
p_drawdown_transects <- ggplot(transects_data) + #, t %in% (c(2, 5) * t_sec))) +
  geom_line(data = transects_data %>% dplyr::filter(years == "5 years") %>% dplyr::select(-years),
            aes(as.numeric(-aquifer_drawdown_ratio), along_stream_distance/5280, linetype = as.factor(x1/5280)), alpha = 0.15) +
  geom_line(aes(as.numeric(-aquifer_drawdown_ratio), along_stream_distance/5280, color = years, linetype = as.factor(x1/5280))) +
  scale_color_discrete("Years after\npumping initiates") +
