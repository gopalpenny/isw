...
r <- x1 # ft
get_s_ratio(r, K, D, V, t_sec)
get_s_ratio(r, K, D, V, t_sec*2)
...
get_depletion_of_well <- function(x1, x2, y, K, D, V, t) {
  
  r_w <- sqrt(y^2 + (x2-x1)^2) # ft
  r_wi <- sqrt(y^2 + (x2+x1)^2) # ft
  
  q_fraction <- get_q_fraction(x1, K, D, V, t) # %
  ds_w <- get_s_ratio(r_w, K, D, V, t) # ft / flowrate
  dw_wi <- -get_s_ratio(r_wi, K, D, V, t) # ft / flowrate
  return(list(q_fraction = q_fraction, s_ratio = ds_w + dw_wi))
}

x2 <- 1e3 # ft
y <- 1e3 # ft
get_depletion_of_well(x1, x2, y, K, D, V, t_sec)
